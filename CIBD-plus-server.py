...
def compute_q1(q3,q4,q2,Xws,q1old):
    #--- Cálculo de q1
    ...
    #q1a = np.array([np.angle((C1+cmath.sqrt(D1[0]))/complex(B1,-A1[0])),np.angle((C1+cmath.sqrt(D1[1]))/complex(B1,-A1[1]))])
    #q1b = np.array([np.angle((C1-cmath.sqrt(D1[0]))/complex(B1,-A1[0])),np.angle((C1-cmath.sqrt(D1[1]))/complex(B1,-A1[1]))])
    
    for i in range(len(q2)):
        q1a = np.angle((C1+cmath.sqrt(D1[i]))/complex(B1,-A1[i]))
        q1b = np.angle((C1-cmath.sqrt(D1[i]))/complex(B1,-A1[i]))
        q1adeg = q1a*180/np.pi
        q1bdeg = q1b*180/np.pi
        print("q1adeg,q1bdeg:\n{},\n{}".format(q1adeg, q1bdeg))
        # Posición a la que llega con q1a y q1b
        # Twa and Twb se definen como la transformación de la muñeca con q1a y q1b
        Twa = WristTransformation(q1a[0],q2[i],q3,q4,0)
        Twb = WristTransformation(q1b[0],q2[i],q3,q4,0)
        ...
        NXw_Pwa = np.linalg.norm(Xw-Pwa)
        NXw_Pwb = np.linalg.norm(Xw-Pwb)
        print("NXw_Pwa,NXw_Pwb: {},{}".format(NXw_Pwa, NXw_Pwb))
        if NXw_Pwa <= NXw_Pwb:
            q1=q1a[0]
            A6temp = Twa
        else:
            ...
            A6temp = Twb
        q1deg = q1*180/np.pi
        print("q1deg[{}]:{}".format(i,q1deg))
        print("q1[{}]:{}".format(i,q1))

        if len(q2) == 1:
            return q1, A6temp, q2[i]
        elif i > 0:
            if abs(q1old - q1) <= abs(q1old - q10):
                return q1, A6temp, q2[i]
            else:
                return q10, A6temp10, q2[0]
        else:
            q10 = q1
            A6temp10 = A6temp
            continue
...
try:
    while True:
        ...
        #parse message from MATLAB
        Rtxt = Rtxt[1:len(Rtxt)-2]
        Rtxt = Rtxt.split()
        print("Rtxt:", Rtxt)
        ...
        try:
            ...
            print("Caught a null char! Getting invalid values in the rot./pos. matrix.")
            continue
        ...
        q2 = compute_q2(q3,q4,Xws)
        print("q2:{}".format(q2))

        q1, A6temp, q2 = compute_q1(q3,q4,q2,Xws,q1)
        print("q1:{}".format(q1))
        print("q2:{}".format(q2))
        ...
        print("A6\n{}".format(A6))
        print("B6\n{}".format(B6))
        print("B6[2,0], d7: {}, {}".format(B6[2,0],d7))
        q6a = np.arccos(min(max(B6[2,0]/d7,-1),1))
        q6b = -np.arccos(min(max(B6[2,0]/d7,-1),1))
        
        # q6a = np.absolute(np.emath.arccos(B6[2,0]/d7))
        # q6b = -np.absolute(np.emath.arccos(B6[2,0]/d7))
        # print("q6a,q6b:{},{}".format(q6a,q6b))
        if -1*np.pi/180 < q6a and q6a < 1*np.pi/180:
            q6a = 1*np.pi/180
        if -1*np.pi/180 < q6b and q6b < 1*np.pi/180:
            q6b = -1*np.pi/180

        q6adeg = q6a*180/np.pi
        q6bdeg = q6b*180/np.pi
        print("q6adeg,q6bdeg:{},{}".format(q6adeg,q6bdeg))
        ...
        print("q5aa,q5ab:{},{}".format(q5aa,q5ab))
        ...
        Teeaa = FinalTransformation(q1,q2,q3,q4,q5aa,q6a)
        Teeab = FinalTransformation(q1,q2,q3,q4,q5ab,q6a)
        Teeba = FinalTransformation(q1,q2,q3,q4,q5ba,q6b)
        Teebb = FinalTransformation(q1,q2,q3,q4,q5bb,q6b)
        ...
        q5adeg = q5a*180/np.pi
        q5bdeg = q5b*180/np.pi
        print("q5adeg,q5bdeg:{},{}".format(q5adeg,q5bdeg))
